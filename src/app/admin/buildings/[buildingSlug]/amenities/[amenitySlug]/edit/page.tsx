...
import { updateAmenityAction } from "@/app/admin/actions";
    ...
    <main className="min-h-screen bg-[#f4faf7]">
      ...
      <div className="mx-auto flex max-w-4xl flex-col gap-8 px-6 py-10 md:px-10">
        <div>
          ...
          <h1 className="mt-2 font-display text-4xl text-[#1e2a27] md:text-5xl">Edit Amenity</h1>
        </div>

        <form
          action={updateAmenityAction}
          className="grid gap-8 rounded-[24px] bg-white px-6 py-8 shadow-[0_4px_24px_rgba(10,109,60,0.1)] md:px-8 md:py-10"
        >
          <input type="hidden" name="buildingSlug" value={building.slug} />
          <input type="hidden" name="currentAmenitySlug" value={amenity.slug} />
          ...
          <section className="grid gap-5">
            ...
            <label className="flex flex-col gap-2 text-sm font-medium text-[#1e2a27]">
              ...
              <textarea
                ...
                rows={6}
                defaultValue={amenity.termsAndConditions ?? ""}
          ...
          <section className="grid gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-[#4a6358]">
                Path only — no domain. Example:{" "}
                <span className="rounded bg-[#eef7f1] px-1.5 py-0.5 font-mono text-xs text-[#0a6d3c]">dsenteu/bbqm</span>
              </p>
            </div>
            <label className="flex flex-col gap-2 text-sm font-medium text-[#1e2a27]">
              Booking Path
              <input
                name="calBookingLink"
                defaultValue={amenity.calBookingLink}
          ...
          <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            ...
            {[0, 1, 2, 3, 4].map((i) => (
              <div key={i} className="flex flex-col gap-2">
                {amenity.images[i] && (
                  <div className="relative aspect-square overflow-hidden rounded-xl bg-[#d4ede0]">
                    <Image
                      src={amenity.images[i]}
                      alt={`Image ${i + 1}`}
                      fill
